...
export function TreatmentForm({ open, onClose, patientId }: { open: boolean; onClose: () => void; patientId: number }) {
  ...
  return <Modal open={open} onClose={close} title="Add treatment"><form onSubmit={submit} className="space-y-4"><Field label="Treatment type" htmlFor="type"><TextInput id="type" value={form.type} onChange={(event) => update("type", event.target.value)} placeholder="e.g. Filling" /></Field><Field label="Date" htmlFor="date"><TextInput id="date" type="date" value={form.date} onChange={(event) => update("date", event.target.value)} /></Field><Field label="Diagnosis" htmlFor="diagnosis"><TextInput id="diagnosis" value={form.diagnosis} onChange={(event) => update("diagnosis", event.target.value)} /></Field><Field label="Notes" htmlFor="notes" hint="Optional"><TextArea id="notes" value={form.notes} onChange={(event) => update("notes", event.target.value)} /></Field><div className="grid grid-cols-2 gap-4"><Field label="Total amount" htmlFor="totalPayment"><TextInput id="totalPayment" type="number" min="0" value={form.totalPayment} onChange={(event) => update("totalPayment", event.target.value)} /></Field><Field label="Amount paid" htmlFor="totalPayed"><TextInput id="totalPayed" type="number" min="0" value={form.totalPayed} onChange={(event) => update("totalPayed", event.target.value)} /></Field></div>{error && <p className="rounded-lg bg-[var(--color-danger-soft)] px-3 py-2 text-[13px] text-[var(--color-danger)]">{error}</p>}<div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={close}>Cancel</Button><Button type="submit" disabled={submitting}>{submitting ? "Saving…" : "Save treatment"}</Button></div></form></Modal>;
}
